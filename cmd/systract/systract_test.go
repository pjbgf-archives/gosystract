...
func TestExtract_E2E_Executable(t *testing.T) {
	...
	expected := []SystemCall{
		...
		{ID: 233, Name: "epoll_ctl"}, {ID: 267, Name: "readlinkat"},
		{ID: 3, Name: "close"}, {ID: 72, Name: "fcntl"},
	}

	actual, err := Extract(NewDumpReader(fileName))

	should.BeNil(err, "should not error for keyring.dump")
	should.HaveSameItems(expected, actual, "should match expected syscalls for keyring.dump")
}
...
func TestIsInitSymbol(t *testing.T) {
	assertThat := func(assumption, assemblyLine string, expectedMatch bool) {
		should := should.New(t)

		ok := isInitSymbol(assemblyLine)

		should.BeEqual(expectedMatch, ok, assumption)
	}

	assertThat("should match init symbols that contains %",
		"TEXT %22%22.init(SB) gofile..<autogenerated>", true)
	assertThat("should match init symbols with letters and dots",
		"TEXT os.init(SB) /usr/local/go/src/os/error.go", true)
	assertThat("should not match symbols that are suffixed with init",
		"TEXT internal/cpu.doinit(SB) /usr/local/go/src/internal/cpu/cpu_x86.go", false)
}

func TestExtractInitSymbols(t *testing.T) {
	assertThat := func(assumption string, symbols map[string]symbolDefinition, expected []string) {
		should := should.New(t)

		initSymbols := extractInitSymbols(symbols)

		should.HaveSameItems(expected, initSymbols, assumption)
	}

	assertThat("should return only init functions",
		map[string]symbolDefinition{
			"runtime.(*gcWork).init":                        symbolDefinition{},
			"github.com/pjbgf/gosystract/cmd/systract.init": symbolDefinition{},
			"internal/cpu.doinit":                           symbolDefinition{},
			"internal/cpu.indexByte":                        symbolDefinition{},
			"reflect.(*funcTypeFixed64).Out":                symbolDefinition{},
		}, []string{"runtime.(*gcWork).init", "github.com/pjbgf/gosystract/cmd/systract.init"})
}
